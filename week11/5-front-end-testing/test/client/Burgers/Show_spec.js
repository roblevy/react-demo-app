...
import BurgerShow from '../../../src/components/burgers/Show';
import { getBasket } from '../../../src/lib/basket';

const testData = {
  _id: 1234,
  name: 'Pretendy Trendy Burger',
...
describe('BurgerShow', () => {
  it('should show the burger name and image', done => {
    const component = mount(<BurgerShow match={match}/>);
  ...
  it('should show an input with quantity 0 by default', done => {
    const component = shallow(<BurgerShow match={match}/>);
    component.setState({ burger: testData });
  ...
  it('should redirect to the /basket page after "Add to basket" click', done => {
    ...
    const component = shallow(<BurgerShow match={match} history={history}/>);
    component.setState({ burger: testData });
  ...
  it('should add the correct item to localStorage on add to basket', done => {
    const component = shallow(<BurgerShow match={match} history={[]}/>);
